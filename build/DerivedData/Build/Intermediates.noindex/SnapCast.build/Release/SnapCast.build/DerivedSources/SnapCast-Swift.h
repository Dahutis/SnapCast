...
#ifndef SNAPCAST_SWIFT_H
#define SNAPCAST_SWIFT_H
...
# undef any
# pragma clang attribute push(__attribute__((external_source_symbol(language="Swift", defined_in="SnapCast",generated_declaration))), apply_to=any(function,enum,objc_interface,objc_category,objc_protocol))
...
/// <code>mouseUp</code> commits it to <code>state.elements</code>. Rendering is delegated to
/// <code>AnnotationRenderer</code> so the live overlay matches the editor exactly.
SWIFT_CLASS("_TtC8SnapCast20AnnotationCanvasView")
...
/// elements and renders them via <code>AnnotationRenderer</code>. The window is visible on
/// screen so screen-capture streams see the strokes naturally — there’s no
/// per-frame compositing trick.
SWIFT_CLASS("_TtC8SnapCast22AnnotationCanvasWindow")
@interface AnnotationCanvasWindow : NSPanel
...
@class NSApplication;
SWIFT_CLASS("_TtC8SnapCast11AppDelegate")
...
SWIFT_CLASS("_TtC8SnapCast21CaptureSessionManager")
...
/// inside the view; <code>scale</code>/<code>origin</code> map between view points and base pixels.
SWIFT_CLASS("_TtC8SnapCast18EditorCanvasNSView")
@interface EditorCanvasNSView : NSView
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)coder OBJC_DESIGNATED_INITIALIZER;
@property (nonatomic, readonly, getter=isFlipped) BOOL flipped;
@property (nonatomic, readonly) BOOL acceptsFirstResponder;
- (void)updateTrackingAreas;
...
@class SCStream;
SWIFT_CLASS("_TtC8SnapCast14FrameProcessor")
@interface FrameProcessor : NSObject <SCStreamOutput>
...
SWIFT_CLASS("_TtC8SnapCast21PostProcessController")
@interface PostProcessController : NSObject <NSWindowDelegate>
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
